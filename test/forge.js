...
describe("forge", function () {
  ...
  it("deploy Forge", async function () {
    let forgeContract = await ethers.getContractFactory("Forge");
    forge = await forgeContract.deploy(stable.address, await stable.decimals(), "200000000"); // 200$ min collateral
  ...
  it("deploy ForgeAsset", async function () {
    ...
    await forge.addAsset(synth.address, oracle.address, false, minCratio, liqCratio);
  ...
  it("deployer open oven", async function () {
    await stable.approve(forge.address, MAX_UINT);
    ...
    await forge.open(deposit, wanted, 0)
    expect(await stable.balanceOf(deployer.address)).to.equal(ethers.BigNumber.from("999000000000"))
    expect(await stable.balanceOf(forge.address)).to.equal(ethers.BigNumber.from(ONE_THOUSAND))
    ...
    expect(await forge.getOvensLength(deployer.address)).to.equal(ethers.BigNumber.from("1"))
  })

  it("deployer close oven", async function () {
    await forge.close(0)
    expect(await forge.getOvensLength(deployer.address)).to.equal(ethers.BigNumber.from("0"))
    expect(await stable.balanceOf(deployer.address)).to.equal(ethers.BigNumber.from(ONE_MILL))
    expect(await stable.balanceOf(forge.address)).to.equal(ethers.BigNumber.from("0"))
    expect(await synth.balanceOf(deployer.address)).to.equal(ethers.BigNumber.from("0"))
  })

  it("user1 open oven", async function () {
    await stable.connect(user1).approve(forge.address, MAX_UINT);
    let deposit = BigInt(ONE_THOUSAND) // 1K
    let cratio = 200000000 // 200% collaterization
    let power = 8+18-6-1 // cratio dec + synth dec - usdc dec - 1 for price
    let wanted = deposit * BigInt(10**power) / BigInt(cratio);
    await forge.connect(user1).open(deposit, wanted, 0)
    ...
    expect(await synth.totalSupply()).to.equal(ethers.BigNumber.from("50000000000000000000"))
    expect(await forge.getOvensLength(user1.address)).to.equal(ethers.BigNumber.from("1"))
  })

  it("user1 close oven", async function () {
    await forge.connect(user1).close(0)
    expect(await forge.getOvensLength(user1.address)).to.equal(ethers.BigNumber.from("0"))
  ...
  it("user2 fail open oven", async function () {
    await stable.connect(user2).approve(forge.address, MAX_UINT);
    let deposit = BigInt(ONE_THOUSAND) // 1K
    let wanted = "76923076923100000000"
    await expect(forge.connect(user2).open(deposit, wanted, 0)).to.be.reverted // mint amount too big
  })
  ...
  it("zero fees", async function () {
    await forge.setFee(0);
    expect (await forge.fee()).to.equal(ethers.BigNumber.from("0"))
  })
  ...
  it("user2 open oven", async function () {
    ...
    let power = 8+18-6-1 // cratio dec + synth dec - usdc dec - 1 for price
    let wanted = deposit * BigInt(10**power) / BigInt(cratio);
    await forge.connect(user2).open(deposit, wanted, 0)
    ...
    expect(await synth.totalSupply()).to.equal(ethers.BigNumber.from("50000000000000000000"))
    expect(await forge.getOvensLength(user2.address)).to.equal(ethers.BigNumber.from("1"))
  })

  it("user2 deposit", async function () {
    let oven = await forge.connect(user2).ovens(user2.address, 0);
    let addedCollateral = oven.collateral.add(ethers.BigNumber.from(ONE_THOUSAND))
    await forge.connect(user2).edit(0, addedCollateral, oven.amount) // deposit 1K
    expect(await synth.balanceOf(user2.address)).to.equal(ethers.BigNumber.from("50000000000000000000"))
  ...
  it("user2 withdraw", async function () {
    ...
    await forge.connect(user2).edit(0, removedCollateral, oven.amount) // withdraw 1K
    expect(await synth.balanceOf(user2.address)).to.equal(ethers.BigNumber.from("50000000000000000000"))
    expect(await stable.balanceOf(user2.address)).to.equal(ethers.BigNumber.from("999000000000"))
  })
  ...
  it("user2 burn", async function () {
    ...
    await forge.connect(user2).edit(0, oven.collateral, removedAsset) // burn 10 XAU
  ...
  it("user2 mint", async function () {
    ...
    await forge.connect(user2).edit(0, oven.collateral, addedAsset) // mint 10 XAU
    expect(await synth.balanceOf(user2.address)).to.equal(ethers.BigNumber.from("50000000000000000000"))
    expect(await stable.balanceOf(user2.address)).to.equal(ethers.BigNumber.from("999000000000"))
    expect(await forge.getOvenRatio(user2.address,0)).to.equal(ethers.BigNumber.from("200000000"))
  })

  it("oracle change price", async function () {
    await oracle.updateAnswer("11000000000000000000");
    expect(await forge.getOvenRatio(user2.address,0)).to.equal(ethers.BigNumber.from("181818181"))
    await oracle.updateAnswer("12000000000000000000");
    expect(await forge.getOvenRatio(user2.address,0)).to.equal(ethers.BigNumber.from("166666666"))
    await oracle.updateAnswer("15000000000000000000");
    expect(await forge.getOvenRatio(user2.address,0)).to.equal(ethers.BigNumber.from("133333333"))
    await oracle.updateAnswer("16000000000000000000");
    expect(await forge.getOvenRatio(user2.address,0)).to.equal(ethers.BigNumber.from("125000000"))
  })

  it("fail liquidate", async function () {
    await expect(forge.connect(user1).liquidate(user2.address, 0, "50000000000000000000")).to.be.reverted // user1 not enough XAU
    ...
    await expect(forge.connect(user1).liquidate(user2.address, 0, "50000000000000000000")).to.be.reverted // c-ratio too high
    await oracle.updateAnswer("17000000000000000000");
    expect(await forge.getOvenRatio(user2.address,0)).to.equal(ethers.BigNumber.from("117647058"))
    await expect(forge.connect(user1).liquidate(user2.address, 0, "49000000000000000000")).to.be.reverted // oven cant go below min collateral
  })

  it("partial liquidation", async function () {
    await forge.connect(user1).liquidate(user2.address, 0, "25000000000000000000")
  ...
  it("full liquidation", async function () {
    ...
    expect(await forge.getOvensLength(user2.address)).to.equal(ethers.BigNumber.from("0"))
    expect(await stable.balanceOf(forge.address)).to.equal(ethers.BigNumber.from("0"))
    expect(await synth.totalSupply()).to.equal(ethers.BigNumber.from("0"))
  })
  ...
  it("user2 deposit & mint", async function () {
    ...
    await forge.connect(user2).edit(0, addedCollateral, addedAsset) // deposit 1K and mint 50 XAU
  ...
  it("user2 withdraw & burn", async function () {
    ...
    await forge.connect(user2).edit(0, removedCollateral, removedAsset) // withdraw 1K and burn 50 XAU
    expect(await synth.balanceOf(user2.address)).to.equal(ethers.BigNumber.from("50000000000000000000"))
    expect(await stable.balanceOf(user2.address)).to.equal(ethers.BigNumber.from("999000000000"))
  })
  return
